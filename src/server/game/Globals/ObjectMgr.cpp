...
void ObjectMgr::LoadMissingKeyChains()
{
    ...
    do
    {
        ...
        sKeyChainStore.AddEntry(id, kce);
...
void ObjectMgr::LoadSceneTemplates()
{
    ...
    QueryResult templates = WorldDatabase.Query("SELECT SceneId, Flags, ScriptPackageID FROM scene_template");

    if (!templates)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 scene templates. DB table `scene_template` is empty.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u scene templates in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
