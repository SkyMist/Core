	...
	if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        for (int team = 0; team < BG_TEAMS_COUNT; ++team)
        {
            ...
            if (m_lastTick[team] > BG_BG_TickIntervals[points])
            {
                ...
                {
                    RewardHonorToTeam(GetBonusHonorFromKill(2), (team == BG_TEAM_ALLIANCE) ? ALLIANCE : HORDE);
...
void BattlegroundBFG::FillInitialWorldStates(ByteBuffer& data)
{
    ...
    for (uint8 node = 0; node < BG_BG_DYNAMIC_NODES_COUNT; ++node)
        data << uint32((m_Nodes[node] == 0) ? 1 : 0) << uint32(BG_BG_OP_NODEICONS[node]);

    // Node occupied states
    for (uint8 node = 0; node < BG_BG_DYNAMIC_NODES_COUNT; ++node)
        for (uint8 i = 1; i < BG_BG_DYNAMIC_NODES_COUNT; ++i)
            data << uint32((m_Nodes[node] == i) ? 1 : 0) << uint32(BG_BG_OP_NODESTATES[node] + plusArray[i]);
    ...
    data  << uint32(ally)  << uint32(BG_BG_OP_OCCUPIED_BASES_ALLY);
    data  << uint32(horde) << uint32(BG_BG_OP_OCCUPIED_BASES_HORDE);

    // Team scores
    data << uint32(BG_BG_MAX_TEAM_SCORE)             << uint32(BG_BG_OP_RESOURCES_MAX);
    data << uint32(BG_BG_WARNING_NEAR_VICTORY_SCORE) << uint32(BG_BG_OP_RESOURCES_WARNING);
    data << uint32(m_TeamScores[BG_TEAM_ALLIANCE])   << uint32(BG_BG_OP_RESOURCES_ALLY);
    data << uint32(m_TeamScores[BG_TEAM_HORDE])      << uint32(BG_BG_OP_RESOURCES_HORDE);
}
...
void BattlegroundBFG::EventPlayerClickedOnFlag(Player* source, GameObject* /*target_obj*/)
{
    ...
    if (node == BG_BG_DYNAMIC_NODES_COUNT)
    {
        TC_LOG_ERROR("bg.battleground", "Player %s (GUID: %u) in Battle for Gilneas fired EventPlayerClickedOnFlag() but isnt near of any flag",source->GetName(),source->GetGUIDLow());
...
void BattlegroundBFG::EndBattleground(uint32 winner)
{
    ...
    if (winner == ALLIANCE)
    {
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 6 : 4), ALLIANCE);
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 4 : 2), HORDE);
    }
    else if (winner == HORDE)
    {
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 6 : 4), HORDE);
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 4 : 2), ALLIANCE);
    }
    else // complete map_end rewards (if no team wins).
    {
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 4 : 2), HORDE);
        RewardHonorToTeam(GetBonusHonorFromKill(isBGWeekend ? 4 : 2), ALLIANCE);
    }
