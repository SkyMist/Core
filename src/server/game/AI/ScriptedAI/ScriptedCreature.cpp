...
SpellInfo const* ScriptedAI::SelectSpell(Unit* target, uint32 school, uint32 mechanic, SelectTargetType targets, uint32 powerCostMin, uint32 powerCostMax, float rangeMin, float rangeMax, SelectEffect effects)
{
    ...
    for (uint32 i = 0; i < CREATURE_MAX_SPELLS; i++)
    {
        ...
        if (powerCostMin && me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell) && me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell)->powerCost < powerCostMin)
            continue;

        if (powerCostMax && me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell) && me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell)->powerCost > powerCostMax)
            continue;

        // Continue if we don't have the mana to actually cast this spell
        if (me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell) &&
        (me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell)->powerCost > (uint32)me->GetPower(Powers(me->ToUnit()->GetSpellPowerEntryBySpell(tempSpell)->powerType))))
            continue;
