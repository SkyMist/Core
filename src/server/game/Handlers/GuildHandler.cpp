...
void WorldSession::HandleGuildBankerActivate(WorldPacket& recvPacket)
{
    ...
    TC_LOG_DEBUG("guild", "CMSG_GUILD_BANKER_ACTIVATE [%s]: Go: %u AllSlots: %u", GetPlayerInfo().c_str(), GUID_LOPART(guid), SendAllSlots);
...
void WorldSession::HandleGuildBankQueryTab(WorldPacket& recvPacket)
{
    ...
    TC_LOG_DEBUG("guild", "CMSG_GUILD_BANK_QUERY_TAB [%s]: Go: [" UI64FMTD "], TabId: %u, AllSlots: %u", GetPlayerInfo().c_str(), guid, tabId, SendAllSlots);
