...
{
    // No Diminishing for non-debuff spells.
    if (spellproto->IsPositive())
        return DIMINISHING_NONE;

    // General Taunt spells handling.
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        {
            if (spellproto->SpellFamilyFlags[1] & 0x80000000)    // Frostbite
                return DIMINISHING_ROOT;
            else if (spellproto->SpellVisual[0] == 12297)        // Shattered Barrier
                return DIMINISHING_ROOT;
            else if (spellproto->SpellIconID == 2939 && spellproto->SpellVisual[0] == 9963) // Deep Freeze
                return DIMINISHING_CONTROLLED_STUN;
            else if (spellproto->SpellIconID == 193)             // Frost Nova / Freeze (Water Elemental)
                return DIMINISHING_CONTROLLED_ROOT;
            else if (spellproto->Id == 33395)                    // Freeze
                return DIMINISHING_CONTROLLED_ROOT;
            else if (spellproto->Id == 122)                      // Frost Nova
                return DIMINISHING_CONTROLLED_ROOT;
            else if (spellproto->SpellFamilyFlags[0] & 0x800000) // Dragon's Breath
                return DIMINISHING_DRAGONS_BREATH;
            else if (spellproto->Id == 82691)                    // Ring of Frost
                return DIMINISHING_DISORIENT;
            else if (spellproto->Id == 118)                      // Polymorph
                return DIMINISHING_DISORIENT;
            else if (spellproto->Id == 31589)                    // Slow
        ...
        {
            if (spellproto->SpellFamilyFlags[0] & 0x2)             // Hamstring - limit duration to 10s in PvP
                return DIMINISHING_LIMITONLY;
            else if (spellproto->SpellFamilyFlags[0] & 0x01000000) // Charge Stun (own diminishing)
        ...
        {
            if ((spellproto->SpellFamilyFlags[0] & 0x80000000) || (spellproto->SpellFamilyFlags[1] & 0x200)) // Curses/etc
                return DIMINISHING_LIMITONLY;
            else if (spellproto->Id == 6358 || spellproto->Id == 132412) // Seduction
                return DIMINISHING_FEAR;
            else if (spellproto->SpellIconID == 1869)                    // Sin and Punishment (Priest spell, don't ask)
                return DIMINISHING_NONE;
        ...
        {
            if (spellproto->SpellFamilyFlags[0] & 0x8)            // Gouge
                return DIMINISHING_DISORIENT;
            else if (spellproto->SpellFamilyFlags[0] & 0x1000000) // Blind
                return DIMINISHING_FEAR;
            else if (spellproto->SpellFamilyFlags[0] & 0x400)     // Cheap Shot
                return DIMINISHING_OPENING_STUN;
            else if (spellproto->SpellIconID == 163) // Crippling poison - Limit to 10 seconds in PvP (No SpellFamilyFlags)
                return DIMINISHING_LIMITONLY;
            else if (spellproto->Id == 3609)         // Paralytic Poison
        ...
        {
            if ((spellproto->SpellFamilyFlags[0] & 0x400) && spellproto->SpellIconID == 538)        // Hunter's Mark
                return DIMINISHING_LIMITONLY;
            else if ((spellproto->SpellFamilyFlags[0] & 0x40000) && spellproto->SpellIconID == 132) // Scatter Shot (own diminishing)
                return DIMINISHING_SCATTER_SHOT;
            else if (spellproto->SpellVisual[0] == 7484 && spellproto->SpellIconID == 20)           // Entrapment (own diminishing)
                return DIMINISHING_ENTRAPMENT;
            else if ((spellproto->SpellFamilyFlags[1] & 0x1000) && spellproto->SpellIconID == 1721) // Wyvern Sting mech. is MECHANIC_SLEEP but dim. is DIMINISHING_DISORIENT
                return DIMINISHING_DISORIENT;
            else if (spellproto->SpellFamilyFlags[0] & 0x8) // Freezing Arrow
                return DIMINISHING_DISORIENT;
        ...
        {
            if (spellproto->SpellFamilyFlags[0] & 0x100000) // Judgement of Justice - limit duration to 10s in PvP
                return DIMINISHING_LIMITONLY;
            else if ((spellproto->SpellFamilyFlags[1] & 0x804000) && spellproto->SpellIconID == 309) // Turn Evil
                return DIMINISHING_FEAR;
        ...
        {
            if (spellproto->SpellFamilyFlags[2] & 0x4000) // Storm, Earth and Fire - Earthgrab
                return DIMINISHING_NONE;
        ...
        {
            if (spellproto->SpellIconID == 2797) // Hungering Cold (no flags)
                return DIMINISHING_DISORIENT;
            else if ((spellproto->SpellFamilyFlags[0] & 0x10000000) && spellproto->SpellIconID == 2285) // Mark of Blood
                return DIMINISHING_LIMITONLY;
...
int32 GetDiminishingReturnsLimitDuration(DiminishingGroup group, SpellInfo const* spellproto)
{
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            if (spellproto->SpellFamilyFlags[1] & 0x1000)     // Wyvern Sting limit to 6 seconds in PvP
                return 6 * IN_MILLISECONDS;
            else if (spellproto->SpellFamilyFlags[0] & 0x400) // Hunter's Mark limit to 120 seconds in PvP
        ...
        case SPELLFAMILY_PALADIN:
        {
            if (spellproto->SpellFamilyFlags[0] & 0x4) // Repentance - limit to 6 seconds in PvP
                return 6 * IN_MILLISECONDS;
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (spellproto->SpellFamilyFlags[1] & 0x8000000)  // Banish - limit to 6 seconds in PvP
                return 6 * IN_MILLISECONDS;
            else if (spellproto->SpellFamilyFlags[2] & 0x800) // Curse of Tongues - limit to 12 seconds in PvP
                return 12 * IN_MILLISECONDS;
            else if (spellproto->SpellFamilyFlags[1] & 0x200) // Curse of Elements - limit to 120 seconds in PvP
                return 120 * IN_MILLISECONDS;
...
bool SpellMgr::IsSpellValid(SpellInfo const* spellInfo, Player* player, bool msg)
{
    ...
    bool ReagentsCheckNeeded = false;
...
void SpellMgr::GetSetOfSpellsInSpellGroup(SpellGroup group_id, std::set<uint32>& foundSpells, std::set<SpellGroup>& usedGroups) const
{
    ...
    for (SpellGroupSpellMap::const_iterator itr = groupSpell.first; itr != groupSpell.second; ++itr)
    {
        ...
        else foundSpells.insert(itr->second);
    }
}
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, bool active) const
{
    ...
    *Dots can trigger if spell doesn't have PROC_FLAG_SUCCESSFUL_NEGATIVE_MAGIC_SPELL and PROC_FLAG_TAKEN_POSITIVE_MAGIC_SPELL.
    *Only Hots can trigger if spell has PROC_FLAG_TAKEN_POSITIVE_MAGIC_SPELL.
    *Only Dots can trigger if spell has both positivity flags or only PROC_FLAG_SUCCESSFUL_NEGATIVE_MAGIC_SPELL.
...
SpellBonusEntry const* SpellMgr::GetSpellBonusData(uint32 spellId) const
{
    ...
    // Not found, try to lookup first spell rank (if it exists).
    ...
    for (uint32 spell = 0; spell < GetSpellInfoStoreSize(); ++spell)
    {
        ...
        for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (entry->Effects[i].Effect == SPELL_EFFECT_SKILL)
            {
                ...
                if (dbc_node.skill != SKILL_RIDING && dbc_node.skill != SKILL_RUNEFORGING)
                    dbc_node.value = 1;
                else if (dbc_node.skill == SKILL_RUNEFORGING) // Panda gives 500 Runeforging from start.
                    dbc_node.value = 500;
                else                                          // Also handles SKILL_RIDING ranks.
...
void SpellMgr::LoadSpellAreas()
{
    ...
    do
    {
        ...
        if (spellArea.auraSpell)
        {
            ...
            // Don't allow autocast chains by auraSpell field (but allow use as alternative if not present).
...
void SpellMgr::LoadSpellClassInfo()
{
    ...
    for (uint8 ClassID = 0; ClassID < MAX_CLASSES; ClassID++)
    {
