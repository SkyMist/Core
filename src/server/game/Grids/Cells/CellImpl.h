...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = SkyMistCore::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = SkyMistCore::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = SkyMistCore::ComputeCellCoord(x + radius, y + radius).normalize();
