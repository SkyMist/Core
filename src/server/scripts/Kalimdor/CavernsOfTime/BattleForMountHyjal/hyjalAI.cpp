...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    SkyMistCore::AllFriendlyCreaturesInGrid creature_check(me);
    SkyMistCore::CreatureListSearcher<SkyMistCore::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <SkyMistCore::CreatureListSearcher<SkyMistCore::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    SkyMistCore::WorldObjectWorker<SkyMistCore::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <SkyMistCore::CreatureListSearcher<SkyMistCore::AllFriendlyCreaturesInGrid>,
