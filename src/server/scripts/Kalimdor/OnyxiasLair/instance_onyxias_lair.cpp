    ...
    struct instance_onyxias_lair_InstanceMapScript : public InstanceScript
    {
        ...
        void FloorEruption(uint64 floorEruptedGUID)
        {
            if (GameObject* pFloorEruption = instance->GetGameObject(floorEruptedGUID))
            {
                ...
                SkyMistCore::GameObjectInRangeCheck check(pFloorEruption->GetPositionX(), pFloorEruption->GetPositionY(), pFloorEruption->GetPositionZ(), 15);
                SkyMistCore::GameObjectListSearcher<SkyMistCore::GameObjectInRangeCheck> searcher(pFloorEruption, nearFloorList, check);
