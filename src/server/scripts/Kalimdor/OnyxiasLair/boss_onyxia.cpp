                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BELLOWING_ROAR: // Phase PHASE_END
                        {
                            ...
                            SkyMistCore::GameObjectInRangeCheck check(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15);
                            SkyMistCore::GameObjectLastSearcher<SkyMistCore::GameObjectInRangeCheck> searcher(me, Floor, check);
