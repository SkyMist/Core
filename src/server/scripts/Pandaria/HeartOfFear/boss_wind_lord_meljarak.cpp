...
enum Spells
{
    ...
    SPELL_BERSERK           = 120207, // 480 seconds or 8 mins Enrage.
    ...
    // The Swarm
    SPELL_FATE_OF_THE_KORT  = 121782, // Share Damage spell for Kor'thik Elite Blademasters.
    SPELL_FATE_OF_THE_SRAT  = 121803, // Share Damage spell for Sra'thik Amber-Trappers.
    SPELL_FATE_OF_THE_ZART  = 121809, // Share Damage spell for Zar'thik Battle-Menders.
    ...
    SPELL_CORR_RESIN_POOL_A = 122125, // Aura for Corrosive Resin Pool, triggers 122125 dmg each sec.
...
public:
    ...
    {
        boss_wind_lord_meljarakAI(Creature* creature) : BossAI(creature, DATA_WIND_LORD_MELJARAK_EVENT), summons(me)
        {
        ...
        void EnterCombat(Unit* /*who*/) OVERRIDE
        {
            ...
            if (instance)
            {
                instance->SetData(DATA_WIND_LORD_MELJARAK_EVENT, IN_PROGRESS);
        ...
        void EnterEvadeMode() OVERRIDE
        {
            ...
            if (instance)
            {
                instance->SetData(DATA_WIND_LORD_MELJARAK_EVENT, FAIL);
        ...
        void JustDied(Unit* /*killer*/) OVERRIDE
        {
            ...
            if (instance)
            {
                instance->SetData(DATA_WIND_LORD_MELJARAK_EVENT, DONE);
                instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me); // Remove
            }
    ...
    private:
        ...
        {
            for (uint8 i = 0; i < 3; i++)
                if (Creature* Blademaster = me->SummonCreature(NPC_KORTHIK_ELITE_BLADEMASTER, AddSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN))
                    AddsCastOnSelf(Blademaster->GetEntry(), SPELL_FATE_OF_THE_KORT);
            for (uint8 i = 3; i < 6; i++)
                if (Creature* trapper = me->SummonCreature(NPC_SRATHIK_AMBER_TRAPPER, AddSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN))
                    AddsCastOnSelf(trapper->GetEntry(), SPELL_FATE_OF_THE_SRAT);
            for (uint8 i = 6; i < 9; i++)
                if (Creature* mender = me->SummonCreature(NPC_ZARTHIK_BATTLE_MENDER, AddSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN))
                    AddsCastOnSelf(mender->GetEntry(), SPELL_FATE_OF_THE_ZART);
        }
        ...
        uint32 GetSpearImpaledAdds()
        {
            ...
            for (std::list<Creature*>::iterator iter = allAdds.begin(); iter != allAdds.end(); iter++)
                if ((*iter)->HasAura(SPELL_IMP_SPEAR))
                    impaledAdds++;
        ...
        void RemoveImpaledAddsAuras(uint32 numberToRemove)
        {
            ...
            for (std::list<Creature*>::iterator iter = allAdds.begin(); iter != allAdds.end(); iter++)
            {
                if ((*iter)->HasAura(SPELL_IMP_SPEAR) && addsToRemoveAuraFrom > 0)
                {
                    (*iter)->RemoveAurasDueToSpell(SPELL_IMP_SPEAR);
            ...
            else if (GetLivingAddCount() == 6)
            {
                for (uint8 i = 0; i < 3; i++)
                {
                    if (EliteBattleMastersDied()) // Summon Elite Battlemasters.
                        me->SummonCreature(NPC_KORTHIK_ELITE_BLADEMASTER, ReinforcementsSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN);
                    else if (AmberTrappersDied()) // Summon Amber Trappers.
                        me->SummonCreature(NPC_SRATHIK_AMBER_TRAPPER, ReinforcementsSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN);
                    else if (BattleMendersDied()) // Summon Battle Menders.
                        me->SummonCreature(NPC_ZARTHIK_BATTLE_MENDER, ReinforcementsSummonPositions[i], TEMPSUMMON_MANUAL_DESPAWN);
                }
            }
            // Two groups died.
            else if (GetLivingAddCount() == 3)
        ...
        void AddsCastOnSelf(uint32 AddEntry, uint32 SpellId)
        {
            std::list<Creature*> adds;

            GetCreatureListWithEntryInGrid(adds, me, AddEntry, 150.0f);
            ...
            for (std::list<Creature*>::iterator iter = adds.begin(); iter != adds.end(); ++iter)
                if ((*iter)->IsAlive())
                    (*iter)->CastSpell(*iter, SpellId, false);
        }
    };
            ...
            while (uint32 eventId = events.ExecuteEvent()) 
            {
                switch (eventId) 
                {
                    ...
                    case EVENT_CORROSIVE_RESIN:
                        ...
                        events.ScheduleEvent(EVENT_CORROSIVE_RESIN, urand(35000, 70000));
                        break;
...
class spell_meljarak_corrosive_resin : public SpellScriptLoader
{
    public:
        ...
        class spell_meljarak_corrosive_resin_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/) OVERRIDE
            {
                // Check target moving.
                ...
                {
                    // Check the aura.
                    if (Aura* resinAura = player->GetAura(SPELL_CORROSIVE_RESIN))
                    {
                        // Remove the stacks.
                        if (resinAura->GetStackAmount() > 1)
                            resinAura->SetStackAmount(resinAura->GetStackAmount() - 1);
                        else
                            player->RemoveAurasDueToSpell(SPELL_CORROSIVE_RESIN);

                        // Summon the pool.
                        player->CastSpell(player, SPELL_CORR_RESIN_POOL_S, false);
                    }
                }
            }
...
// Fate of the Korthik - 121774.
class BattlemasterCheck : public std::unary_function<Unit*, bool>
{
    public:
        explicit BattlemasterCheck(Unit* _caster) : caster(_caster) { }

        bool operator()(WorldObject* object)
        {
            return object->GetEntry() != NPC_KORTHIK_ELITE_BLADEMASTER;
        }
...
class spell_fate_of_the_korthik : public SpellScriptLoader
{
public:
    spell_fate_of_the_korthik() : SpellScriptLoader("spell_fate_of_the_korthik") { }

    class spell_fate_of_the_korthikSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_fate_of_the_korthikSpellScript);

        void FilterTargets(std::list<WorldObject*>& targets) OVERRIDE
        {
            targets.remove_if(BattlemasterCheck(GetCaster()));
        }

        void Register() OVERRIDE
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_fate_of_the_korthikSpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_fate_of_the_korthikSpellScript();
    }
};

// Fate of the Srathik - 121802.
class TrapperCheck : public std::unary_function<Unit*, bool>
{
    public:
        explicit TrapperCheck(Unit* _caster) : caster(_caster) { }

        bool operator()(WorldObject* object)
        {
            return object->GetEntry() != NPC_SRATHIK_AMBER_TRAPPER;
        }
...
class spell_fate_of_the_srathik : public SpellScriptLoader
{
public:
    spell_fate_of_the_srathik() : SpellScriptLoader("spell_fate_of_the_srathik") { }

    class spell_fate_of_the_srathikSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_fate_of_the_srathikSpellScript);

        void FilterTargets(std::list<WorldObject*>& targets) OVERRIDE
        {
            targets.remove_if(TrapperCheck(GetCaster()));
        }

        void Register() OVERRIDE
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_fate_of_the_srathikSpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_fate_of_the_srathikSpellScript();
    }
};

// Fate of the Zarthik - 121807.
class MenderCheck : public std::unary_function<Unit*, bool>
{
    public:
        explicit MenderCheck(Unit* _caster) : caster(_caster) { }

        bool operator()(WorldObject* object)
        {
            return object->GetEntry() != NPC_ZARTHIK_BATTLE_MENDER;
        }
...
class spell_fate_of_the_zarthik : public SpellScriptLoader
{
public:
    spell_fate_of_the_zarthik() : SpellScriptLoader("spell_fate_of_the_zarthik") { }

    class spell_fate_of_the_zarthikSpellScript : public SpellScript
    {
        PrepareSpellScript(spell_fate_of_the_zarthikSpellScript);

        void FilterTargets(std::list<WorldObject*>& targets) OVERRIDE
        {
            targets.remove_if(MenderCheck(GetCaster()));
        }

        void Register() OVERRIDE
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_fate_of_the_zarthikSpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_fate_of_the_zarthikSpellScript();
    }
};
...
void AddSC_boss_wind_lord_meljarak()
{
    ...
    new spell_fate_of_the_korthik();
    new spell_fate_of_the_srathik();
    new spell_fate_of_the_zarthik();
}
