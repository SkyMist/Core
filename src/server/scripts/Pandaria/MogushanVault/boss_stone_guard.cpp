                    ...
                    case EVENT_MAIN_ATTACK:
                    {
                        if (isInTrueForm)
                        {
                            switch (me->GetEntry())
                            {
                                ...
                                case NPC_JASPER:
                                {
                                    for (uint8 i = 0; i < (Is25ManRaid() ? 2 : 1); ++i)
                                    {
                                    ...
                                    events.ScheduleEvent(EVENT_MAIN_ATTACK, 30000);
                                    break;
                                }
                            }
                        }
            ...
            void DealDamage()
            {
                if (Unit* target = GetHitUnit())
                {
                    if (AuraPtr aura = target->GetAura(SPELL_JASPER_CHAINS))
                    {
                        ...
                        if (damage && stacks)
                            SetHitDamage(damage + damage * (stacks * 0.15)); // every stack got increased 15% damage
                    }
                }
            }
