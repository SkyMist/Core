        ...
        class spell_dru_lifebloom_AuraScript : public AuraScript
        {
            ...
            void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    // restore mana
                    int32 returnMana = CalculatePct(caster->GetCreateMana(), caster->GetSpellPowerEntryBySpell(GetSpellInfo())->powerCostPercentage) * stack / 2;
                ...
                if (Unit* target = GetUnitOwner())
                {
                    if (AuraEffect const* aurEff = GetEffect(EFFECT_1))
                    {
                        ...
                        if (Unit* caster = GetCaster())
                        {
                            ...
                            // restore mana
                            int32 returnMana = CalculatePct(caster->GetCreateMana(), caster->GetSpellPowerEntryBySpell(GetSpellInfo())->powerCostPercentage) * dispelInfo->GetRemovedCharges() / 2;
